    ...
    <el-form ref="formRef" 
		...
		<template v-for="(field, index) in formFields" :key="index">
		  <template v-if="field.type === 'line'">
			  <el-divider :content-position="field.align || 'center'">{{field.label}}</el-divider>
		  </template>
		  <template v-else>
			<el-form-item 
				class="u-form_item"
				...
				<VuSelect
					...
					:placeholder="field.placeholder || `请选择${getDefaultPlaceHolder(field, formValue)}`"
					:size="field.size"
					:componentOption="field.componentOption"
					@create="field.create"
					:clearable="field.clearable ?? true"
					:disabled="field.disabled"
					v-model="formValue[field.name]">
				...
				<div class="u-form-item_tips" v-html="field.tips" v-if="field.tips && showTips">
				</div>
			</el-form-item>
		  </template>
		</template>
...
<script setup>
	...
	const props = defineProps({
		...
		inlineTips: { type: Boolean, default: false },
	...
	const formValue = ref(computed(() => props.value))

	const arrFields = ['checkbox-group']
	const objFields = ['number-range']
	// 初始化
	formFields.value.map(field => {
		if(arrFields.indexOf(field.type) >= 0 && types.isNullOrUndefined(formValue.value[field.name])) {
			formValue.value[field.name] = []
			return
		}
		
		if(objFields.indexOf(field.type) >= 0 && types.isNullOrUndefined(formValue.value[field.name])) {
			formValue.value[field.name] = {}
			return
		}
	})
	...
	const getDefaultPlaceHolder = (field, dataValue) => {
		if(types.isFunction(field.label)) {
			return field.label(dataValue[field.name], dataValue);
		}
		return `${field.label}`
	}
