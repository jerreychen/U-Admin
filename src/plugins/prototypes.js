...
export default {
	install(app) {
		...
		String.prototype.format = function() {
			...
			let result = this;
			// 如果参数只有一个，并且是数组或者对象
			if (arguments.length == 1 && (Array.isArray(arguments[0]) || Vue.prototype.isObject(arguments[0]))) { 
		...
		Date.prototype.between = function(startDate, endDate, options) {
			if(!(Vue.prototype.isDate(startDate) && Vue.prototype.isDate(endDate))) {
				return false
			}
